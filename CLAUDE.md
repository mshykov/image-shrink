...
- **Sparkle is optional at build time and required at release time.** `fetch-sparkle.sh` puts
  it in `vendor/` (gitignored, pinned by version *and* SHA-256); the sources compile either way
  behind `#if canImport(Sparkle)`, and without it the menu item opens the releases page. A
  release build must have it, or installed copies never hear about the next version.
- **The bundle is assembled in an order that matters.** The Quick Actions are generated by
  *running the binary that was just built* (`--list-presets`), so everything it links has to be
  inside the bundle first — Sparkle included. Embedding the framework after that step cost three
  of the five Finder actions, silently, because the generator's `while read` loop simply got no
  input. It now fails loudly instead. Signing then runs inside-out: XPC services, Autoupdate,
  Updater.app, the framework, and only then the app.
...
- **The app's only network traffic is the update check.** That is a promise made on the site
  and in SECURITY.md, and it constrains what may be added: no analytics, no crash reporting, no
