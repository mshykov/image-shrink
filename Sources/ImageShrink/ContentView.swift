    ...
    private func button(_ label: String, value: Double, selected: Bool) -> some View {
        Button { if !selected { apply(value) } } label: {
            Text(label)
                ...
                // The capsule behind a pill is drawn by the container, so the button itself is
                // just text with transparent padding — and SwiftUI hit-tests what is drawn.
                // Without this, only the digits answered a click and the rest of the pill was
